 ...
 *    
 */
public class LogingMDCFilter implements Filter {
    ...
    private void mdc(HttpServletRequest hsr) { 
        ...
        MDC.put(MDCConstants.REQUEST_ID_HEADER, hsr.getHeader(MDCConstants.REQUEST_ID_HEADER));
        MDC.put(MDCConstants.REQUEST_SEQ_HEADER, hsr.getHeader(MDCConstants.REQUEST_SEQ_HEADER));
//        MDC.put(MDCConstants.REQUEST_ID_MDC_KEY,hsr.getHeader(MDCConstants.REQUEST_ID_HEADER));
       
//        MDC.put(MDCConstants.REQUEST_ID_MDC_KEY, String.valueOf(Thread.currentThread().getId()));
...
//        MDC.put(MDCConstants.REQUEST_SEQ_MDC_KEY,requestSeq);
        ...
        MDCUtils.RidAndRseqHandle();

//        MDC.put(MDCConstants.TIMESTAMP, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSS").format(LocalDate.now()));
        MDC.put(MDCConstants.TIMESTAMP, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new Date()));
        ...
        if(mappedHeaders) {
            ...
            if(e != null) {
                while (e.hasMoreElements()) {
                    ...
                    value = NameValueFilter(header,value,hsr.getRequestedSessionId());
        ...
        if(mappedCookies) {
            ...
            if(cookies != null && cookies.length > 0) {
                for(Cookie  cookie : cookies) {
                    ...
                    value = NameValueFilter(name,value,hsr.getRequestedSessionId());
        ...
        if(mappedParameters) {
            ...
            if(e != null) {
                while (e.hasMoreElements()) {
                    ...
                    value = NameValueFilter(key,value,hsr.getRequestedSessionId());
     ...
     * 去除或替换敏感字段，防止记录到日志中
     */
    private String NameValueFilter(String name,String value,String sessionId){
